...
class SinglePHP {
    ...
    public static function getInstance($conf){
        if(!(self::$_instance instanceof self)){// instanceof 判断其左边对象是否为其右边类的实例
            self::$_instance = new self($conf);
        }
        return self::$_instance;//返回一个对象
    }
    /**
     * 运行应用实例
     * @access public
    ...
    public function run(){
        ...
        C('APP_FULL_PATH', getcwd().'/'.C('APP_PATH').'/');//获取和设置配置参数
        includeIfExist( C('APP_FULL_PATH').'/common.php');
        $pathMod = C('PATH_MOD');
        $pathMod = empty($pathMod)?'NORMAL':$pathMod;
        spl_autoload_register(array('SinglePHP', 'autoload'));//自动加载未定义类
        ...
        call_user_func(array($controller,$this->a.'Action'));//call_user_func:把第一个参数作为回调函数使用
    }
...
class DB {
    ...
    {
//        mysqli_query($this->_db, "SET AUTOCOMMIT=0");
        return mysqli_begin_transaction($this->_db);
    }

    public function rollback()
    {
        return mysqli_query($this->_db, "ROLLBACK");
    }

    public function commit()
    {
        return mysqli_commit($this->_db);
    }
...
//        }
     ...
     * 查询，用于select语句
     * @param string $sql 要查询的sql
     * @return bool|obj 查询成功返回对应对象，失败返回false
     */
